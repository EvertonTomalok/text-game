...
func TestProcessQuestion(t *testing.T) {
	...
	testCases := []TestMainQuestionCase{
		{
			...
			ExpectedComplementaryId: 1, // key of map in shared.ComplementaryQuestions. First MainQuestion links to the ComplementaryQuestions of key 1 and 2
		},
		{
			IndexMainQuestion:       1,
	...
	for _, testCase := range testCases {
		...
		if complementaryQuestion.ID != testCase.ExpectedComplementaryId {
			t.Errorf("the ComplementaryQuestion.ID expected was %d but received %d", testCase.ExpectedComplementaryId, complementaryQuestion.ID)
		}
	}
}
